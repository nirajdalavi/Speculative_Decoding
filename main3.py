...
def speculative_generate_one_prompt(
    ...
    # Keep timing barriers minimal: synchronize once before and once after timed region.
    ...
    while generated < max_new_tokens:
        ...
        # Transfer the whole proposed chunk once to target GPU (lower transfer overhead).
        proposed_chunk_on_target = torch.cat(proposed_tokens, dim=1).to(target_device, non_blocking=True)

        # Parallel verification over the chunk in one target forward pass.
        verify_out = target_model(
            input_ids=proposed_chunk_on_target,
            past_key_values=target_past,
            use_cache=True,
        )
        verify_logits_chunk = verify_out.logits

        # Predicted token ids for each proposed position:
        # pos0 uses existing target_logits; pos i>0 uses verify_logits_chunk[:, i-1, :].
        pred_first = torch.argmax(target_logits, dim=-1, keepdim=True)  # [B,1]
        if n_propose > 1:
            pred_rest = torch.argmax(verify_logits_chunk[:, :-1, :], dim=-1)  # [B,n-1]
            predicted_ids = torch.cat([pred_first, pred_rest], dim=1)  # [B,n]
        else:
            predicted_ids = pred_first

        matches = proposed_chunk_on_target.eq(predicted_ids)
        mismatch_positions = (~matches).nonzero(as_tuple=False)

        if mismatch_positions.numel() == 0:
            # Entire chunk accepted.
            accepted_prefix = n_propose
            generated += accepted_prefix
            accepted_total += accepted_prefix

            draft_past = draft_states[accepted_prefix]
            draft_logits = cur_logits
            target_past = verify_out.past_key_values
            target_logits = verify_logits_chunk[:, -1, :]
            continue

        # First mismatch index in proposed chunk.
        mismatch_index = int(mismatch_positions[0, 1].item())
        accepted_prefix = mismatch_index
        mismatch_token = predicted_ids[:, mismatch_index : mismatch_index + 1]  # [B,1] on target device

        generated += accepted_prefix + 1
        accepted_total += accepted_prefix + 1

        # Update draft cache from checkpoint at accepted prefix plus replacement token.
        draft_past = draft_states[accepted_prefix]
        draft_past, draft_logits = step_model(
            draft_model,
            mismatch_token.to(draft_device, non_blocking=True),
            draft_past,
        )

        # Rebuild target cache through accepted prefix + mismatch token in one pass.
        if accepted_prefix > 0:
            commit_tokens = torch.cat(
                [proposed_chunk_on_target[:, :accepted_prefix], mismatch_token],
                dim=1,
            )
        else:
            commit_tokens = mismatch_token

        commit_out = target_model(
            input_ids=commit_tokens,
            past_key_values=target_past,
            use_cache=True,
        )
        target_past = commit_out.past_key_values
        target_logits = commit_out.logits[:, -1, :]
